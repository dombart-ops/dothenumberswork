/**
 * dothenumberswork.com — Real Estate Deal Analysis Engine (TypeScript port)
 *
 * Implements 23 investment methodologies across 5 categories:
 *   1. Old-School Rules of Thumb
 *   2. Investor-Grade Financial Metrics
 *   3. Strategy-Specific Analysis
 *   4. Risk and Stress Testing
 *   5. Comparative and Educational
 *
 * Ported from /legacy/analyzer.js. Pure functions, zero side effects, server-safe.
 */
...
export type Verdict = "PASS" | "MARGINAL" | "FAIL" | "EXCELLENT" | "STRONG" | "MODERATE" | "WEAK";
...
export interface RentTest {
  decline: number;
  cashFlow: number;
  isNegative: boolean;
}

export interface CapexTest {
  amount: number;
...
export interface StressTests {
  vacancy: VacancyTest[];
  interestRate: RateTest[];
  rentDecline: RentTest[];
  capex: CapexTest[];
  breakEvenVacancy: number;
}

export interface IRRProjection {
  irr: number;
  cashFlows: number[];
...
export interface OpportunityCost {
  realEstate: OpportunityCostComparison;
  stocks: OpportunityCostComparison;
  hysa: OpportunityCostComparison;
  bonds: OpportunityCostComparison;
  reits: OpportunityCostComparison;
  wins: "real_estate" | "stocks";
}
...
export interface DealMetrics {
  ...
  cashFlowPerDoor: number;
  cashFlowPerDoor50: number;
  roi: number;
  irrProjection: IRRProjection;
  brrrr: BRRRRAnalysis;
  flip: FlipAnalysis;
...
const percent = (num: number): string => (num * 100).toFixed(2);
const dollar = (num: number): string => Math.round(num).toLocaleString();
...
export function calculateDealMetrics(inputs: DealInputs): DealMetrics {
  ...
  // First-year principal/interest split
  ...
  const totalCashInvested =
    downPayment + closingCosts + rehabCost + holdingCostsMonthly * rehabMonths;
  ...
  const breakEvenRatio =
    annualEffectiveGrossIncome > 0
      ? (annualOperatingExpenses + annualMortgagePayment) / annualEffectiveGrossIncome
      : 0;
  ...
  // === 50% RULE ===
  const estimatedOperatingExpenses50 = monthlyGrossRent * 0.5;
...
export function calculateIRRValue(cashFlows: number[]): number {
...
function calculateBRRRR({
  ...
  loanTerm,
  annualCashFlow,
}: BRRRRInputs): BRRRRAnalysis {
  const allInCost =
    purchasePrice + rehabCost + holdingCostsMonthly * rehabMonths + closingCosts;
  ...
  if (interestRate > 0) {
    newMonthlyPayment =
      (newLoanAmount * (monthlyRate * Math.pow(1 + monthlyRate, payments))) /
      (Math.pow(1 + monthlyRate, payments) - 1);
  }

  const newAnnualCashFlow =
    annualCashFlow + (loanAmount - newLoanAmount > 0 ? loanAmount - newLoanAmount : 0);
  const newCashOnCash = cashLeftIn > 0 ? newAnnualCashFlow / cashLeftIn : 999;

  const equityCreated = arv - allInCost;

  const verdict: Verdict =
    cashLeftIn <= 0 ? "PASS" : cashLeftIn / allInCost < 0.2 ? "MARGINAL" : "FAIL";
...
function calculateFlip({
  ...
  const netProfit =
    arv - purchasePrice - rehabCost - holdingCosts - closingCosts - sellingCosts;
  ...
  const verdict: Verdict =
    netProfit >= allInCost * 0.15
      ? "PASS"
      : netProfit >= allInCost * 0.10
      ? "MARGINAL"
      : "FAIL";
...
function calculateStressTests({
  ...
  totalCashInvested,
  holdingPeriod,
}: StressInputs): StressTests {
  const vacancyTests: VacancyTest[] = [0.10, 0.15, 0.20, 0.25].map((rate) => {
  ...
  const breakEvenVacancy =
    vacancyTests.find((t) => t.isNegative) ?? vacancyTests[vacancyTests.length - 1];

  const rateTests: RateTest[] = [1, 2, 3].map((increase) => {
  ...
  const rentTests: RentTest[] = [0.05, 0.10, 0.15].map((decline) => {
  ...
  const capexTests: CapexTest[] = [10000, 25000, 50000].map((amount) => {
    const monthsToRecover = amount / Math.max(monthlyCashFlow, 1);
    const irrImpact = totalCashInvested > 0 ? amount / totalCashInvested / holdingPeriod : 0;
    return {
      amount,
...
function calculateOpportunityCost({
  ...
  const totalPrincipal = firstYearPrincipal * holdingPeriod;
  const realEstateEnd = totalCashFlow + totalPrincipal + propertyValueEnd;

  const stocksEnd = totalCashInvested * Math.pow(1.10, holdingPeriod);
  const hysaEnd = totalCashInvested * Math.pow(1.045, holdingPeriod);
  const bondsEnd = totalCashInvested * Math.pow(1.04, holdingPeriod);
  const reitsEnd = totalCashInvested * Math.pow(1.075, holdingPeriod);

  return {
    realEstate: {
      totalReturn: round2(realEstateEnd - totalCashInvested),
      finalValue: round2(realEstateEnd),
    },
    stocks: {
      totalReturn: round2(stocksEnd - totalCashInvested),
      finalValue: round2(stocksEnd),
    },
    hysa: {
      totalReturn: round2(hysaEnd - totalCashInvested),
      finalValue: round2(hysaEnd),
    },
    bonds: {
      totalReturn: round2(bondsEnd - totalCashInvested),
      finalValue: round2(bondsEnd),
    },
    reits: {
      totalReturn: round2(reitsEnd - totalCashInvested),
      finalValue: round2(reitsEnd),
    },
...
function calculateCompositeScore({
  ...
  irrProjection,
  stressTests,
}: CompositeInputs): CompositeScore {
  let score = 0;
    ...
    breakdown: {
      cashOnCash: Math.min(
        ...
        cashOnCashReturn >= 0.10
          ...
          ? 12
          : Math.round((cashOnCashReturn / 0.06) * 8)
      ),
      capRate: Math.min(
        15,
        capRate >= 0.08 ? 15 : capRate >= 0.05 ? 8 : Math.round((capRate / 0.05) * 5)
      ),
      dscr: Math.min(15, dscr >= 1.25 ? 15 : dscr >= 1.0 ? 8 : Math.round(dscr * 5)),
      cashFlowPerDoor: Math.min(
        10,
        cashFlowPerDoor >= 200
          ? 10
          : cashFlowPerDoor >= 100
          ? 7
          : Math.round((cashFlowPerDoor / 20) * 5)
      ),
      onePercent: Math.min(
        10,
        onePercentRule >= 0.01
          ? 10
          : onePercentRule >= 0.0075
          ? 5
          : Math.round((onePercentRule / 0.0075) * 4)
      ),
      breakEven: Math.min(
        10,
        breakEvenRatio <= 0.80
          ...
          : breakEvenRatio <= 0.90
          ? 5
          : Math.round((1 - breakEvenRatio) * 50)
      ),
      irr: Math.min(
        10,
        irrProjection.irr >= 0.15
          ? 10
          : irrProjection.irr >= 0.08
          ? 5
          : Math.round((irrProjection.irr / 0.08) * 4)
      ),
...
export function generateMethodologyResults(
  metrics: DealMetrics,
  inputs: DealInputs
): MethodologyResult[] {
  const { investmentStrategy, arv = 0, rehabCost = 0, numberOfUnits, propertyType } = inputs;
  ...
  results.push({
    ...
    threshold: "≥ 1.0%",
    verdict:
      metrics.onePercentRule >= 0.01 ? "PASS" : metrics.onePercentRule >= 0.0075 ? "MARGINAL" : "FAIL",
    explanation: `This property generates ${percent(
      metrics.onePercentRule
    )}% of its purchase price in monthly rent. ${
      metrics.onePercentRule >= 0.01
        ? "This meets the classic 1% rule threshold, suggesting strong cash flow potential."
        : metrics.onePercentRule >= 0.0075
        ? "This is close to the 1% rule but falls slightly short. Still worth deeper analysis."
        : "This falls below the 1% rule threshold. In expensive markets this can still work, but you'll need to rely on appreciation."
    }`,
  });
  ...
  results.push({
    ...
    threshold: "≥ 2.0%",
    verdict:
      metrics.twoPercentRule >= 0.02 ? "PASS" : metrics.twoPercentRule >= 0.015 ? "MARGINAL" : "FAIL",
    explanation: `At ${percent(metrics.twoPercentRule)}%, this ${
      metrics.twoPercentRule >= 0.02
        ? "hits the aggressive 2% rule, common in Midwest cash flow markets."
        : metrics.twoPercentRule >= 0.015
        ? "approaches the 2% rule but doesn't quite make it. Still solid for many markets."
        : "doesn't reach the 2% rule. This is typical in appreciation-focused markets."
    }`,
  });
  ...
  results.push({
    ...
    threshold: "≥ $100/door",
    verdict:
      metrics.cashFlowPerDoor50 >= 200
        ? "PASS"
        : metrics.cashFlowPerDoor50 >= 100
        ? "MARGINAL"
        : "FAIL",
    explanation: `Using the conservative 50% expense rule, you'd net approximately $${Math.round(
      metrics.cashFlowPerDoor50
    )}/door/month. ${
      metrics.cashFlowPerDoor50 >= 200
        ? "This is a comfortable margin."
        : metrics.cashFlowPerDoor50 >= 100
        ? "This meets the minimum threshold for most investors."
        : "This is tight, any vacancy or large repair could put you in the red."
    }`,
  });
  ...
  if (rehabCost > 0 && arv > 0) {
    ...
    results.push({
      ...
      explanation: `Your Maximum Allowable Offer would be $${Math.round(
        mao
      ).toLocaleString()}. You're ${
        inputs.purchasePrice > mao
          ? "$" + Math.round(inputs.purchasePrice - mao).toLocaleString() + " OVER"
          : "$" + Math.round(mao - inputs.purchasePrice).toLocaleString() + " UNDER"
      } that mark. ${
        inputs.purchasePrice <= mao
          ? "This protects your profit margin nicely."
          : percentAboveMAO <= 10
          ? "Slightly over but within acceptable range for the right deal."
          : "Significantly over, you're risking your profit cushion."
      }`,
    });
  }
  ...
  results.push({
    ...
    threshold: "≥ $200/door",
    verdict:
      metrics.cashFlowPerDoor >= 200
        ? "PASS"
        : metrics.cashFlowPerDoor >= 100
        ? "MARGINAL"
        : "FAIL",
    explanation: `Each unit would put approximately $${Math.round(
      metrics.cashFlowPerDoor
    )}/month in your pocket. ${
      metrics.cashFlowPerDoor >= 200
        ? "This hits the comfort zone for experienced investors."
        : metrics.cashFlowPerDoor >= 100
        ? "This meets the minimum threshold most landlords require."
        : "This is below the typical minimum, consider renegotiating or walking."
    }`,
  });
  ...
  results.push({
    ...
    explanation: `At ${metrics.grm.toFixed(2)}, you'd need ${metrics.grm.toFixed(
      1
    )} years of gross rent to equal the purchase price. ${
      metrics.grm <= 8
        ? "This is a strong GRM indicating good value."
        : metrics.grm <= 12
        ? "This is typical for many markets but not exceptional."
        : "This is a high GRM, you're paying a premium for this property."
    }`,
  });
  ...
  results.push({
    ...
    explanation: `Your unleveraged return is ${percent(metrics.capRate)}%. ${
      metrics.capRate >= 0.08
        ? "This is a strong cap rate that would attract institutional interest."
        : metrics.capRate >= 0.05
        ? "This is moderate, typical for many residential markets."
        : "This is a low cap rate. You're likely banking on appreciation rather than cash flow."
    }`,
  });
  ...
  results.push({
    ...
    threshold: "≥ 10%",
    verdict:
      metrics.cashOnCashReturn >= 0.10
        ? "PASS"
        : metrics.cashOnCashReturn >= 0.06
        ? "MARGINAL"
        : "FAIL",
    explanation: `Your cash investment returns ${percent(
      metrics.cashOnCashReturn
    )}% annually. ${
      metrics.cashOnCashReturn >= 0.10
        ? "This exceeds the 10% target most investors seek."
        : metrics.cashOnCashReturn >= 0.06
        ? "This is acceptable but not exceptional."
        : "This is below the typical 6% minimum. Consider reallocating your capital."
    }`,
  });
  ...
  results.push({
    ...
    threshold: "> $0",
    verdict: metrics.annualNOI > 0 ? "PASS" : "FAIL",
    explanation: `This property generates $${Math.round(
      metrics.annualNOI
    ).toLocaleString()} annually before debt service. ${
      metrics.annualNOI > 0
        ? "The property can support itself without financing."
        : "The property cannot cover its own operating expenses, a major red flag."
    }`,
  });
  ...
  results.push({
    ...
    explanation: `The property's income covers its debt ${metrics.dscr.toFixed(2)}x. ${
      metrics.dscr >= 1.25
        ? "This meets bank requirements with a healthy cushion."
        : metrics.dscr >= 1.0
        ? "The property covers its mortgage but leaves little room for error."
  ...
  results.push({
    ...
    explanation: `Your total annualized return (cash + equity + appreciation) is ${percent(
      metrics.roi
    )}%. ${
      metrics.roi >= 0.15
        ? "This is an excellent all-in return."
        : metrics.roi >= 0.08
        ? "This is a solid return when you count all wealth-building mechanisms."
        : "This is below the 15% target. The deal may work for appreciation but not total return."
    }`,
  });
  ...
  results.push({
    ...
    threshold: "≥ 15%",
    verdict:
      metrics.irrProjection.irr >= 0.15
        ? "PASS"
        : metrics.irrProjection.irr >= 0.08
        ? "MARGINAL"
        : "FAIL",
    explanation: `Your projected 5-year IRR is ${percent(metrics.irrProjection.irr)}%. ${
      metrics.irrProjection.irr >= 0.15
        ? "This is institutional-grade, private equity would be interested."
        : metrics.irrProjection.irr >= 0.08
        ? "This is a reasonable IRR for a residential deal."
        : "This IRR is low. Your capital might work harder elsewhere."
    }`,
  });
  ...
  results.push({
    ...
    threshold: "≤ 80%",
    verdict:
      metrics.breakEvenRatio <= 0.80
        ? "PASS"
        : metrics.breakEvenRatio <= 0.90
        ? "MARGINAL"
        : "FAIL",
    explanation: `${percent(
      metrics.breakEvenRatio
    )}% of your rental income goes to expenses and mortgage. ${
      metrics.breakEvenRatio <= 0.80
        ? "You have a healthy 20%+ cushion for vacancies and repairs."
        : metrics.breakEvenRatio <= 0.90
        ? "Your cushion is thin but workable."
        : "WARNING: Over 90% of income is committed. One bad month and you're negative."
    }`,
  });
  ...
  if (rehabCost > 0 && arv > 0) {
    ...
    results.push({
      ...
      value: `${percent(brrrr.recoveryPercent / 100)}% capital recovered`,
      numericValue: brrrr.recoveryPercent,
      threshold: "≥ 100% recovery",
      verdict: brrrr.verdict,
      explanation: `You can pull out $${Math.round(
        brrrr.capitalRecovered
      ).toLocaleString()} of your $${Math.round(brrrr.allInCost).toLocaleString()} investment (${percent(
        brrrr.recoveryPercent / 100
      )}%). ${
        brrrr.cashLeftIn <= 0
          ? "PERFECT: You recover all (or more than) your capital."
          : brrrr.recoveryPercent >= 80
          ? "Strong recovery, most of your capital comes back out."
          : "Limited capital recovery. This may not work as a BRRRR."
      }`,
    });
  }
  ...
  if (rehabCost > 0 && arv > 0 && investmentStrategy === "fix_and_flip") {
    ...
    results.push({
      ...
      explanation: `Net profit: $${Math.round(
        flip.netProfit
      ).toLocaleString()} on $${Math.round(flip.allInCost).toLocaleString()} all-in. ${
        flip.roi >= 0.15
          ? "This is a solid flip margin."
          : flip.roi >= 0.10
          ? "Margins are thin but may work with experience."
          : "WARNING: Margins are too thin. One surprise and you're underwater."
      }`,
    });
  }
    ...
    results.push({
      ...
      verdict:
        metrics.monthlyCashFlow >= 0
          ? "PASS"
          : metrics.monthlyCashFlow >= -500
          ? "MARGINAL"
          : "FAIL",
      explanation: `${
        metrics.monthlyCashFlow >= 0
          ? "You could live for FREE (or make money) with this house hack."
          : metrics.monthlyCashFlow >= -500
          ? `Your housing cost would be $${Math.abs(
              Math.round(metrics.monthlyCashFlow)
            )}/month, likely below market rent.`
          : "Even as a house hack, this costs more than typical rent."
      }`,
    });
  }
  ...
  results.push({
    ...
    verdict:
      metrics.stressTests.breakEvenVacancy >= 20
        ? "PASS"
        : metrics.stressTests.breakEvenVacancy >= 15
        ? "MARGINAL"
        : "FAIL",
    explanation: `This deal goes cash-flow negative at ${Math.round(
      metrics.stressTests.breakEvenVacancy
    )}% vacancy. ${
      metrics.stressTests.breakEvenVacancy >= 20
        ? "Strong cushion, you can handle long vacancies."
        : metrics.stressTests.breakEvenVacancy >= 15
        ? "Moderate cushion but stay alert."
        : "Tight, a single vacancy could cause problems."
    }`,
  });
  ...
  results.push({
    ...
    value: metrics.stressTests.interestRate[2].isNegative
      ? `Negative at +${
          metrics.stressTests.interestRate.find((t) => t.isNegative)
            ? (metrics.stressTests.interestRate.find((t) => t.isNegative) as RateTest).rate -
              inputs.interestRate
            : 3
        }%`
      : "Survives +3%",
    ...
    verdict: !metrics.stressTests.interestRate[1].isNegative
      ? "PASS"
      : !metrics.stressTests.interestRate[0].isNegative
      ? "MARGINAL"
      : "FAIL",
    explanation: `If rates rise, this deal ${
      !metrics.stressTests.interestRate[2].isNegative
        ? "remains positive even at +3%. Very rate-resilient."
        : metrics.stressTests.interestRate[1].isNegative
        ? "goes negative quickly. Highly rate-sensitive."
        : "survives +1% but fails at +2%. Moderate sensitivity."
    }`,
  });
  ...
  results.push({
    ...
    value:
      metrics.opportunityCost.wins === "real_estate" ? "BEATS stocks" : "LOSER vs stocks",
    numericValue:
      metrics.opportunityCost.realEstate.totalReturn -
      metrics.opportunityCost.stocks.totalReturn,
    threshold: "Beat S&P 500",
    verdict: metrics.opportunityCost.wins === "real_estate" ? "PASS" : "FAIL",
    explanation: `Over 5 years, real estate returns $${Math.round(
      metrics.opportunityCost.realEstate.totalReturn
    ).toLocaleString()} vs $${Math.round(
      metrics.opportunityCost.stocks.totalReturn
    ).toLocaleString()} in the S&P 500. ${
      metrics.opportunityCost.wins === "real_estate"
        ? "Real estate wins, this deal beats the stock market."
        : "Stocks win, you'd make more in an index fund."
    }`,
  });
  ...
  results.push({
    ...
    explanation: generateCompositeExplanation(composite),
  });

  return results;
}

function generateCompositeExplanation(composite: CompositeScore): string {
...
// ============================================================================
// SHAREABLE ID ENCODING (stateless, base64url)
// ============================================================================

/**
 * Pack the deal inputs into a URL-safe id so /results/[id] is shareable
 * without needing a database. Format: "v1." + base64url(JSON.stringify(inputs)).
 */
export function encodeAnalysisId(inputs: DealInputs): string {
...
export function decodeAnalysisId(id: string): DealInputs | null {
  if (!id.startsWith("v1.")) return null;
  const urlSafe = id.slice(3);
